...
project_id int,status_id int,created_date timestamp default current_timestamp,remarks text);


alter table tbl_project_status add FOREIGN key(project_id)REFERENCES tbl_projects(id);


alter table tbl_projects_status add FOREIGN key(status_id)REFERENCES  mst_project_status(id);
...
alter table tbl_projects_employees add FOREIGN key(employee_id)REFERENCES tbl_employees(id);